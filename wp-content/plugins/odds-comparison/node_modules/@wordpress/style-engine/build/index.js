...
function compileCSS(style, options = {}) {
  ...
  if (!options?.selector) {
    const inlineRules = [];
    rules.forEach(rule => {
      inlineRules.push(`${(0, _changeCase.paramCase)(rule.key)}: ${rule.value};`);
    });
  ...
  const selectorRules = Object.keys(groupedRules).reduce((acc, subSelector) => {
    acc.push(`${subSelector} { ${groupedRules[subSelector].map(rule => `${(0, _changeCase.paramCase)(rule.key)}: ${rule.value};`).join(' ')} }`);
    return acc;
  }, []);
