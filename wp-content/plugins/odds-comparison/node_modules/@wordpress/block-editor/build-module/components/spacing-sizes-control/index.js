 ...
 */
import { BaseControl, __experimentalHStack as HStack } from '@wordpress/components';
...
export default function SpacingSizesControl({
  ...
  const renderControls = () => {
    if (view === VIEWS.axial) {
      return createElement(AxialInputControls, { ...inputControlProps
      });
    }

    if (view === VIEWS.custom) {
      return createElement(SeparatedInputControls, { ...inputControlProps
      });
    }

    return createElement(SingleInputControl, {
      side: view,
      ...inputControlProps,
  ...
  const label = sprintf( // translators: 2. Type of spacing being modified (Padding, margin, etc). 1: The side of the block being modified (top, bottom, left etc.).
  __('%1$s %2$s'), labelProp, sideLabel).trim();
  const dropdownLabelText = sprintf( // translators: %s: The current spacing property e.g. "Padding", "Margin".
  ...
  }, label), !hasOneSide && !hasOnlyAxialSides && createElement(SidesDropdown, {
