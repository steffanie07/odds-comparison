...
<html>
  ...
  useEffect(() => cleanup, [cleanup]); // We need to counter the margin created by scaling the iframe. If the scale
  // is e.g. 0.45, then the top + bottom margin is 0.55 (1 - scale). Just the
  // top or bottom margin is 0.55 / 2 ((1 - scale) / 2).

  const marginFromScaling = contentHeight * (1 - scale) / 2;
  return createElement(Fragment, null, tabIndex >= 0 && before, createElement("iframe", { ...props,
    style: { ...props.style,
...
function IframeIfReady(props, ref) {
  const isInitialised = useSelect(select => select(blockEditorStore).getSettings().__internalIsInitialized, []); // We shouldn't render the iframe until the editor settings are initialised.
